...
public class PaymentApplication {
	...
	private static final int POLL_INTERVAL = 7000; 			// Max interval to check for new jobs
	private static final int WORKER_REQUEST_TIMEOUT = 8000; 	// Set the request timeout for activate job request used to poll for new job. Default 0. if the requestTimeout < 0, long polling is disabled and the request is completed immediately, even when no job is activated
	private static final int WORKER_TIMEOUT = 5000; 		// Set the time for how long a job is exclusively assigned for this worker.
	...
	private static void loadProperties() {
       Properties properties = new Properties();
        try (FileInputStream input = new FileInputStream(ZEEBE_PROPERTIES_PATH)) {
            ...
            //ZEEBE_AUTHORIZATION_SERVER_URL = properties.getProperty("authorization.server.url");
